...
function updateDashboard() {
  ...
  document.getElementById('kpi-sales-sub').innerText = `from ${formatInteger(orders)} orders`;
  
  document.getElementById('kpi-profit').innerText = formatEuro(profit);
  document.getElementById('kpi-profit-sub').innerText = `${formatEuro(cost)} product costs`;
  ...
  document.getElementById('kpi-customers-sub').innerText = `${formatInteger(qty)} products sold`;
  
  document.getElementById('kpi-return-rate').innerText = formatPercent(returnRate);
  document.getElementById('kpi-return-rate-sub').innerText = `${formatInteger(totalReturning)} customers returned`;
  
  document.getElementById('kpi-aov').innerText = formatEuro(aov);
  document.getElementById('kpi-aov-sub').innerText = `${(qty / (orders || 1)).toFixed(1)} products per order`;
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          backgroundColor: '#0f172a',
          callbacks: {
            label: context => ` ${context.label}: ${formatInteger(context.raw)} customers`
          }
        }
      }
    }
...
function renderChannelMix(b2cSales, b2bSales, b2cCusts, b2bCusts, b2cOrds, b2bOrds) {
  ...
  document.getElementById('channel-sub-b2c').innerText = `${formatInteger(b2cCusts)} customers - ${formatInteger(b2cOrds)} orders`;
  
  document.getElementById('channel-val-b2b').innerText = formatEuro(b2bSales);
  document.getElementById('channel-sub-b2b').innerText = `${formatInteger(b2bCusts)} resellers - ${formatInteger(b2bOrds)} orders`;
}
...
function getNextBestAction(item) {
  ...
  if (item.sales > 250000 && item.lastActiveDays < 30) {
    ...
    className = "vip";
    reason = `Reseller has a very high revenue of ${formatEuro(item.sales)} and was active in the past 30 days. VIP treatment is recommended to consolidate loyalty.`;
    ...
    className = "incentive";
    reason = `Tot nu toe slechts ${item.orders} orders geplaatst. Bied een welkomst- of winkelwagenkorting aan om de bestelfrequentie op gang te brengen.`;
  } else if (item.orders > 7 && item.orders < 12 && item.lastActiveDays <= 30) {
    title = "Aanbeveling cross-selling";
    className = "cross";
    reason = `Recent actief (afgelopen 30 dagen) met een gemiddelde frequentie van ${item.orders} orders. Stel complementaire productcategorieën voor.`;
  } else if (item.sales > 120000) {
    title = "Volumekorting reseller";
    className = "discount";
    reason = `B2B partner heeft een jaarlijks volume van ${formatEuro(item.sales)} bereikt. Bied volumekortingen aan om grotere orders te stimuleren.`;
  } else if (item.sales < 50000) {
    title = "Kortingen beperken en marge behouden";
    className = "margin";
    reason = `Revenuevolume is laag (${formatEuro(item.sales)}). Limit further discounts to protect gross margin.`;
  } else {
  ...
  const candidates = [
    {
      ...
      priority: "high",
      title: "Customer retention campaign",
      headline: `Retention of ${formatInteger(riskCount)} risicocustomers`,
      desc: `Als we 10% van de risicocustomers behouden, levert dit geschat ${formatEuro(riskCount * 0.10 * aov)} extra revenue.`,
    ...
    },
    {
      ...
      desc: `If 10% of one-time buyers place a second order, this generates ${formatEuro(singleOrderCount * 0.10 * aov)} extra omzet.`,
    ...
    },
    {
      ...
      headline: "Increase gross profit margin to 15%",
      desc: `Increasing the profit margin to 15% (currently %${margin.toFixed(1)}) generates ${formatEuro(sales * Math.max(0, 15 - margin) / 100)} extra gross profit.`,
    ...
    },
    {
      ...
      desc: `Increasing the average reseller order size by 5% generates ${formatEuro(b2bSales * 0.05)} extra revenue.`,
...
- Total revenue: ${formatEuro(sales)} (B2B Reseller: ${formatEuro(b2bSales)}, B2C Individual: ${formatEuro(b2cSales)})
- Total net profit: ${formatEuro(profit)} (Profitmarge: %${margin.toFixed(1)})
- Aantal orders: ${formatInteger(orders)} (Gemiddelde bestelwaarde / AOV: ${formatEuro(aov)})
...
function getFallbackAIResponse(userText) {
  ...
  let findings = [
    {
      title: "Current revenue & profit situation",
      metric: `Revenue: ${formatEuro(sales)}, Net profit: ${formatEuro(profit)}`,
      explanation: `Over the selected period, the gross profit margin is %${margin.toFixed(1)} and the average order value (AOV) is ${formatEuro(aov)}.`
    }
  ];
  let actions = [
    {
  ...
  let limitations = [
    ...
    "Human rights, NPS, support tickets, or detailed customer profiles are not available in this dataset."
  ];

  if (text.includes("sales") || text.includes("drop") || text.includes("why") || text.includes("revenue")) {
    summary = "Lokaliseerde factoren die van invloed zijn op de verkoopdaling.";
    findings = [
      {
        title: "Marge- en bestelvolume-krimp",
        metric: `Profitmarge: %${margin.toFixed(1)}`,
        explanation: "A decline in order frequency and lower product margins limit total revenue growth."
      }
    ];
    actions = [
      {
  ...
  } else if (text.includes("risk") || text.includes("churn") || text.includes("churn") || text.includes("customer")) {
    summary = "Customer churn risks evaluated based on purchase frequency and recency.";
    findings = [
      {
        title: "Minder frequente kopers",
        metric: "Risico-segment",
        explanation: "Een aanzienlijk deel van het customersbestand heeft de afgelopen 90 tot 180 dagen geen aankoop gedaan."
      }
    ];
    actions = [
      {
    ...
    findings = [
      {
        ...
        explanation: "Revenue is mainly driven by top regions, providing opportunities for underperforming markets."
      }
    ];
    actions = [
      {
        ...
        reason: "Kopieer succesvolle marketingcampagnes from Noord-Amerika naar Europa en Pacific.",
...
2. Basisschema van jouw antwoord MOET strikt een geldig JSON-object zijn. Zorg ervoor dat het exact voldoet aan de volgende JSON-structuur:
{
  ...
  "findings": [
    {
      ...
      "metric": "Ondersteunende metric from de context (bijv: $15M omzet, %13,2 winstmarge)",
  ...
  "limitations": [
    "Dataset limitations (e.g.: Customer names, NPS scores, support tickets, and product descriptions are missing from the dataset)"
  ]
}
3. Verzin geen getallen of gegevens bfromen de verstrekte context. Speculeer niet over ontbrekende data.
4. Elke bevinding (finding) moet direct gelinkt zijn aan een dashboard metric from de context.`;
...
1. Baseer je fromslfromend op de berekende cijfers from de prompt. Verzin zelf geen nieuwe cijfers.
2. Schrijf een beknopt, krachtig strategisch commentaar in het Nederlands (maximaal 4-5 zinnen).
3. Leg from welke operationele stappen prioritair zijn om dit scenario te realiseren (bijvoorbeeld versterken van B2B resellers, inzet van retentie-e-mails voor risicogroepen).
4. Gebruik een professionele en directie-waardige toon.`;

      const promptText = `Beoordeel de volgende gesimuleerde scenario-fromkomsten:
...
1. Doel herhalingsaankopen: %${simData.targetRepeatRate.toFixed(1)} (Bijdrage: +${formatEuro(simData.repeatImpact)} Revenue)
2. Doel klantbehoud (risicogroepen): %${simData.churnRecoverRate.toFixed(1)} (Bijdrage: +${formatEuro(simData.churnImpact)} Revenue)
3. Doel brutowinstmarge: %${simData.targetMargin.toFixed(1)} (Bijdrage: +${formatEuro(simData.marginImpact)} Brutowinst)
4. Doel stijging AOV: +%${simData.aovIncreasePct.toFixed(1)} (Bijdrage: +${formatEuro(simData.aovImpact)} Revenue)
...
Geef een strategische en beknopte evaluatie in het Nederlands op basis van deze waarden.`;
      ...
      const fallbackCommentary = `<strong>Offline analyse-rapport:</strong> Voor de geselecteerde doelen is een totale potentiële impact van <strong>${formatEuro(simData.totalImpact)}</strong> berekend. Het verhogen van de herhaalaankopen naar %${simData.targetRepeatRate.toFixed(1)} en het behouden van %${simData.churnRecoverRate.toFixed(1)} van de risicocustomers vormen de belangrijkste hefbomen voor groei. Operationeel moeten we inzetten op gerichte volumekortingen voor B2B resellers en automatische e-mailcampagnes voor risicovolle B2C-customers. Om de marge van %${simData.targetMargin.toFixed(1)} te behalen, moeten kortingen op minder winstgevende producten worden afgebouwd.`;
