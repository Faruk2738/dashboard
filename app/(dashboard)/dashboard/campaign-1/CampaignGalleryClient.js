...
const campaigns = [
  { segment: 'Champions', filter: 'Champions', title: 'AdventureWorks Ambassador', eyebrow: 'Referral campaign', offer: 'Earn $20 credit for every successful referral', detail: 'Your friend receives 10% off their first order', image: '/campaigns/ambassador-champions-en.png', position: 'center', fit: 'contain', accent: '#16a34a', audience: 1742, Icon: UsersRound },
  { segment: 'At Risk', filter: 'At Risk', title: 'Ride Again', eyebrow: 'Win-back campaign', offer: 'Save 10% on 2 accessories or 15% on 3+', detail: 'Free delivery on qualifying orders', image: '/campaigns/ride-again-at-risk-en.png', position: 'center', fit: 'contain', accent: '#65a30d', audience: 3238, Icon: RefreshCw },
  { segment: 'Loyal Customers', filter: 'Loyal', title: 'Ride More. Earn More.', eyebrow: 'Tiered loyalty campaign', offer: 'Unlock Bronze, Silver & Gold rewards', detail: 'Earn points, early access and VIP benefits', image: '/campaigns/loyalty-loyal-en.png', position: '17% 16%', accent: '#d97706', audience: 3259, Icon: Medal },
  { segment: "Can't Lose", filter: 'Lost', title: 'Why You Are Special', eyebrow: 'High-value recovery', offer: 'Personal advice and curated bike bundles', detail: 'Exclusive benefits reserved for you', image: '/campaigns/special-cant-lose-en.png', position: '82% 14%', accent: '#4d7c0f', audience: 1039, Icon: Sparkles },
  { segment: 'New & Potential Loyalists', filter: 'New', title: 'Thanks for Riding With Us', eyebrow: 'Welcome campaign', offer: '10% off your next accessory purchase', detail: 'Bike care tips and hand-picked essentials', image: '/campaigns/welcome-new-en.png', position: '16% 15%', accent: '#0284c7', audience: 3286, audienceNote: '772 New + 2,514 Potential Loyalists', Icon: Gift },
  { segment: 'Hibernating', filter: 'Lost', title: 'Last Chance Trade-In', eyebrow: 'Reactivation campaign', offer: 'Trade in old gear and unlock a special price', detail: 'Limited-time offer — while stock lasts', image: '/campaigns/last-chance-hibernating-en.png', position: 'center', fit: 'contain', accent: '#ca8a04', audience: 3115, Icon: TimerReset },
];
  ...
  return <div className="min-h-full bg-gradient-to-br from-[#f7fbff] via-[#fffdf8] to-[#f4faf6] p-3 text-slate-800 lg:p-5"><div className="mx-auto max-w-[1120px]">
    ...
    <div className="mt-3 grid gap-3 lg:grid-cols-[180px_minmax(0,1fr)]">
      ...
      {campaigns.map((campaign) => { const { segment, filter, title, image, position, fit, accent } = campaign; const selected = filters.rfmSegment === filter; return <article key={segment} onClick={() => toggle('rfmSegment', filter)} className={`group cursor-pointer overflow-hidden rounded-2xl border bg-white text-left shadow-[0_8px_24px_rgba(35,65,95,.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_16px_34px_rgba(35,65,95,.14)] ${selected ? 'border-blue-400 ring-2 ring-blue-200' : 'border-slate-200'}`}>
        <div className="relative h-[145px] overflow-hidden bg-[#f7f4ec]"><Image src={image} alt={`${title} campaign for ${segment}`} fill unoptimized sizes="(max-width:768px) 100vw, (max-width:1024px) 50vw, 38vw" className={`${fit === 'contain' ? 'object-contain' : 'object-cover'} transition duration-500 group-hover:scale-[1.02]`} style={{ objectPosition: position }} /><div className="absolute inset-0 bg-gradient-to-t from-white/15 via-transparent to-transparent" /><span className="absolute bottom-2.5 left-3 rounded-full border border-white/70 bg-white/90 px-2.5 py-1 text-[8px] font-black uppercase tracking-wider text-slate-700 shadow-sm backdrop-blur-sm">{segment}</span></div>
    ...
    {selectedCampaign && <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/60 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`${selectedCampaign.title} campaign image`} onClick={() => setSelectedCampaign(null)}><div className="flex h-[96vh] w-full max-w-[1500px] flex-col overflow-hidden rounded-2xl border border-white bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="relative min-h-0 flex-1 bg-[#f5f2e9]"><Image src={selectedCampaign.image} alt={`${selectedCampaign.title} full campaign artwork`} fill unoptimized sizes="1500px" className="object-contain" preload /><button type="button" onClick={() => setSelectedCampaign(null)} className="absolute right-4 top-4 rounded-full border border-slate-200 bg-white/95 p-2 text-slate-700 shadow-md hover:bg-white" aria-label="Close campaign image"><X size={20} /></button></div><div className="flex items-center justify-between gap-4 border-t border-slate-200 bg-white px-5 py-3"><div><p className="text-[9px] font-black uppercase tracking-wider" style={{ color: selectedCampaign.accent }}>{selectedCampaign.segment}</p><h2 className="text-[17px] font-black text-[#123c78]">{selectedCampaign.title}</h2></div><button type="button" onClick={() => { toggle('rfmSegment', selectedCampaign.filter); setSelectedCampaign(null); }} className="flex shrink-0 items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-black text-white shadow-sm" style={{ background: selectedCampaign.accent }}>SELECT CAMPAIGN <ArrowRight size={14} /></button></div></div></div>}
  </div></div>;
}
