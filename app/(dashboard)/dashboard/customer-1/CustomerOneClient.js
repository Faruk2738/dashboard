...
import { Area, AreaChart, Bar, BarChart, CartesianGrid, Cell, LabelList, Legend, Pie, PieChart, ReferenceLine, ResponsiveContainer, Scatter, ScatterChart, Tooltip, XAxis, YAxis, ZAxis } from 'recharts';
...
function RfmSegmentLabel({ cx, cy, midAngle, innerRadius, outerRadius, percent, payload }) {
  if (!percent || percent < 0.06) return null;
  const radians = -midAngle * (Math.PI / 180);
  const radiusPosition = payload?.segment === 'Lost' ? 0.5 : 0.54;
  const radius = innerRadius + (outerRadius - innerRadius) * radiusPosition;
  const x = cx + radius * Math.cos(radians);
  const y = cy + radius * Math.sin(radians);
  return <text x={x} y={y} fill="#fff" stroke="rgba(15,23,42,.38)" strokeWidth="2" paintOrder="stroke" fontSize="9" fontWeight="900" textAnchor="middle" dominantBaseline="central">{Math.round(percent * 100)}%</text>;
}

function SegmentRevenueLabel({ x, y, width, height, value, index }) {
  return <text x={x + width + 5} y={y + height / 2} fill={colors[index % colors.length]} stroke="#fff" strokeWidth="2.2" paintOrder="stroke" fontSize="8.5" fontWeight="900" textAnchor="start" dominantBaseline="central">{money(value)}</text>;
}
...
export default function CustomerOneClient({ data: initialData }) {
  ...
  return <div className="min-h-full bg-[#f7f8fa] p-3 text-slate-800 lg:p-5"><div className="mx-auto max-w-[1040px]">
    ...
    <div className="mt-2 grid gap-2 lg:grid-cols-3">
      ...
      <Panel title="Revenue Contribution by Segment" subtitle="Estimated revenue weighted by customer value"><div className="h-[215px] p-2"><ResponsiveContainer><BarChart data={segmentRevenue} layout="vertical" margin={{ top: 5, right: 48, bottom: 0, left: 4 }}><CartesianGrid stroke="#e5e7eb" strokeDasharray="3 3" horizontal={false} /><XAxis type="number" tick={{ fontSize: 9 }} tickFormatter={money} /><YAxis type="category" dataKey="segment" width={82} tick={{ fontSize: 9, fontWeight: 800, fill: '#334155' }} /><Tooltip content={<Tip />} /><Bar dataKey="revenue" name="Revenue" radius={[0, 5, 5, 0]} onClick={(entry) => toggle('rfmSegment', entry?.segment || entry?.payload?.segment)} className="cursor-pointer">{segmentRevenue.map((item, index) => <Cell key={item.segment} fill={colors[index]} />)}<LabelList dataKey="revenue" content={<SegmentRevenueLabel />} /></Bar></BarChart></ResponsiveContainer></div></Panel>
