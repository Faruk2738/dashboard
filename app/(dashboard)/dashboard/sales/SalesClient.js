...
function Tip({ active, payload, label }) {
  ...
  return <div className="rounded border border-slate-300 bg-white px-2.5 py-1.5 text-[10px] shadow-lg"><b>{label}</b>{payload.map((item) => <p key={item.dataKey} style={{ color: item.color || item.fill }}>{item.name}: {typeof item.value === 'number' ? money(item.value) : item.value}</p>)}</div>;
}
